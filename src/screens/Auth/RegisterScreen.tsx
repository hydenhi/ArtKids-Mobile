...
export default function RegisterScreen({ navigation }: any) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || error.response?.data?.error || 'Đăng ký thất bại. Email hoặc username có thể đã tồn tại.';
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.formContainer}>
          ...
          <Text style={styles.subtitle}>Bắt đầu hành trình sáng tạo cùng bé</Text>
          ...
          <TouchableOpacity style={styles.loginLink} onPress={() => navigation.goBack()} disabled={isLoading}>
...
const styles = StyleSheet.create({
  ...
  formContainer: { flex: 1, justifyContent: 'center', paddingHorizontal: 30, marginTop: 20 },
  appName: { fontSize: 36, fontWeight: '900', color: '#D63031', textAlign: 'center', marginBottom: 5 },
  subtitle: { fontSize: 16, color: '#636E72', textAlign: 'center', marginBottom: 40 },
  input: { backgroundColor: '#FFF', borderRadius: 15, paddingVertical: 15, paddingHorizontal: 20, fontSize: 16, marginBottom: 15, borderWidth: 1, borderColor: '#FDCB6E', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  registerButton: { backgroundColor: '#0984E3', borderRadius: 15, paddingVertical: 15, alignItems: 'center', marginTop: 10, shadowColor: '#0984E3', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 5, elevation: 5 },
