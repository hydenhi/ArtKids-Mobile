...
export default function AllCombosScreen({ route, navigation }: any) {
  ...
  const fetchCombos = async () => {
    try {
      ...
      const params: any = {
        ...
        status: "published", // Chỉ lấy combo đã xuất bản
        limit: 1000,
      };
      const res = await axiosClient.get("/combos", { params });
    ...
    >
      <Image
        source={{
          uri:
            item.thumbnail ||
            item.courses?.[0]?.thumbnail || // Fallback
            "https://placehold.co/400x300/E0F7FA/006064?text=ArtKids+Combo",
        }}
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Tất cả Combo</Text>
        <View style={{ width: 40 }} />
      </View>
