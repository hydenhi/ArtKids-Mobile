  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        console.log('🔍 Dữ liệu Profile trả về:\n', JSON.stringify(response.data, null, 2));
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.profileCard}>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.fullname}>{profile?.fullname || profile?.name || 'Phụ huynh ArtKids'}</Text>
          <Text style={styles.email}>{profile?.email || 'Chưa cập nhật email'}</Text>
          <Text style={styles.username}>@{profile?.username || 'user'}</Text>
        </View>
        <TouchableOpacity style={styles.editButton} onPress={() => Alert.alert('Tính năng', 'Sắp ra mắt!')}>
      ...
      <View style={styles.menuContainer}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('EditProfile', { currentProfile: profile })}
        >
...
const styles = StyleSheet.create({
  ...
  centerContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#FDFBF7' },
  ...
  profileCard: { flexDirection: 'row', backgroundColor: '#FFF', marginHorizontal: 20, padding: 20, borderRadius: 20, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 4 },
  avatar: { width: 70, height: 70, borderRadius: 35, backgroundColor: '#F5F5F5' },
  infoBox: { flex: 1, marginLeft: 15 },
  fullname: { fontSize: 18, fontWeight: 'bold', color: '#2D3436', marginBottom: 4 },
  email: { fontSize: 14, color: '#636E72', marginBottom: 2 },
  username: { fontSize: 13, color: '#0984E3', fontWeight: 'bold' },
  editButton: { width: 40, height: 40, backgroundColor: '#E3F2FD', borderRadius: 20, justifyContent: 'center', alignItems: 'center' },

  menuContainer: { backgroundColor: '#FFF', marginHorizontal: 20, marginTop: 25, borderRadius: 20, paddingVertical: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 4 },
  menuItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 15, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: '#F5F6FA' },
  ...
  logoutButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFEAA7', marginHorizontal: 20, marginTop: 30, paddingVertical: 16, borderRadius: 20, shadowColor: '#FDCC6E', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 3 },
  logoutText: { color: '#D63031', fontSize: 18, fontWeight: 'bold', marginLeft: 10 },
});