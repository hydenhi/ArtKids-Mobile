...
export default function MyCoursesScreen({ navigation }: any) {
  ...
  const fetchMyCourses = async () => {
    try {
      ...
      if (enrolledList.length > 0) {
        console.log(`🔍 Tìm thấy ${enrolledList.length} ID khóa học. Đang tải chi tiết...`);
        ...
        const coursePromises = enrolledList.map(async (item: any) => {
          ...
          const courseId = typeof item.course === 'string' ? item.course : item.course?._id || item;
          
          try {
            // Gọi API lấy thông tin chi tiết của khóa học đó
            const detailRes = await axiosClient.get(`/courses/${courseId}`);
            const courseData = detailRes.data?.course || detailRes.data?.data || detailRes.data;
        ...
        const fullCourses = (await Promise.all(coursePromises)).filter(c => c !== null);
  ...
  const renderCourseItem = ({ item }: { item: any }) => {
    ...
    const thumbnail = item.thumbnail || item.course?.thumbnail || 'https://images.unsplash.com/photo-1580582932707-520aed937b7b?q=80&w=400';
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate('CourseDetail', { courseId: courseId })}
      >
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Hãy tiếp tục hành trình sáng tạo nào!</Text>
      </View>

      {loading ? (
        <View style={styles.centerContainer}>
          <ActivityIndicator size="large" color="#00B894" />
          <Text style={{ marginTop: 10, color: '#78909C' }}>Đang dọn dẹp bàn học...</Text>
        </View>
      ) : (
        <FlatList
          data={myCourses}
          keyExtractor={(item, index) => item._id || item.course?._id || index.toString()}
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptySubText}>Bé hãy nhờ bố mẹ mua khóa học để bắt đầu nhé!</Text>
              
              <TouchableOpacity style={styles.exploreButton} onPress={() => navigation.navigate('Khám phá')}>
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 15, color: '#55E6C1', marginTop: 5, fontWeight: '600' },
  centerContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  listContainer: { paddingHorizontal: 20, paddingBottom: 30 },
  
  card: { flexDirection: 'row', backgroundColor: '#FFF', borderRadius: 20, padding: 12, marginBottom: 15, alignItems: 'center', shadowColor: '#00B894', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 3 },
  thumbnail: { width: 90, height: 90, borderRadius: 15, backgroundColor: '#F5F5F5' },
  infoBox: { flex: 1, marginLeft: 15, justifyContent: 'center' },
  title: { fontSize: 16, fontWeight: 'bold', color: '#37474F', marginBottom: 8 },
  
  progressRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  progressBarBg: { flex: 1, height: 6, backgroundColor: '#E8F5E9', borderRadius: 3, marginRight: 10, overflow: 'hidden' },
  progressBarFill: { height: '100%', backgroundColor: '#00B894', borderRadius: 3 },
  progressText: { fontSize: 12, fontWeight: 'bold', color: '#00B894' },
  
  studyButton: { backgroundColor: '#E8F5E9', paddingVertical: 8, borderRadius: 10, alignItems: 'center' },
  ...
  emptyText: { fontSize: 20, color: '#37474F', fontWeight: 'bold', marginBottom: 5 },
  emptySubText: { fontSize: 14, color: '#90A4AE', textAlign: 'center', paddingHorizontal: 20, marginBottom: 20 },
  exploreButton: { backgroundColor: '#FFCC80', paddingVertical: 12, paddingHorizontal: 25, borderRadius: 20 },
